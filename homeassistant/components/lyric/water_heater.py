"""Support for lyric water heaters."""
...
async def async_setup_entry(
    ...
    lyric = hass.data[DOMAIN][entry.entry_id]
    coordinator: DataUpdateCoordinator[Lyric] = hass.data[DOMAIN][entry.entry_id]

    devices = lyric.session.deviceList.get("water_heater")
    entities = []
    if devices:
        for dev in devices:
            entities.append(HiveWaterHeater(lyric, dev))
...
class HiveWaterHeater(HiveEntity, WaterHeaterEntity):
    ...
    async def async_turn_on(self, **kwargs):
        """Turn on hotwater."""
        await self.lyric.hotwater.setMode(self.device, "MANUAL")

    @refresh_system
    async def async_turn_off(self, **kwargs):
        """Turn on hotwater."""
        await self.lyric.hotwater.setMode(self.device, "OFF")

    @refresh_system
    async def async_set_operation_mode(self, operation_mode: str) -> None:
        """Set operation mode."""
        new_mode = HASS_TO_HIVE_STATE[operation_mode]
        await self.lyric.hotwater.setMode(self.device, new_mode)

    @refresh_system
    async def async_hot_water_boost(self, time_period, on_off):
        """Handle the service call."""
        if on_off == "on":
            await self.lyric.hotwater.setBoostOn(self.device, time_period)
        elif on_off == "off":
            await self.lyric.hotwater.setBoostOff(self.device)

    async def async_update(self) -> None:
        """Update all Node data from Hive."""
        await self.lyric.session.updateData(self.device)
        self.device = await self.lyric.hotwater.getWaterHeater(self.device)
